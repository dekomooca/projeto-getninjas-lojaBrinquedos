...
<html lang="pt-br">
  <head>
    ...
    <title>Categorias - Loja de Brinquedos</title>
  </head>
  <body>
    <nav class="navbar navbar-light">
      ...
      <div class="viewport">
		    ...
		    <div class="modal-dialog" role="document">
		        <div class="modal-content">
		            <div class="modal-header">
		                <h5 class="modal-title" id="confirmacaoExclusaoModalLabel">Exclusão de Categoria</h5>
		                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
		                    <span aria-hidden="true">&times;</span>
		                </button>
		            </div>
		            <div class="modal-body">
		                Deseja excluir a categoria e todos brinquedos cadastrados nela?
		            </div>
        ...
        <a href="ServletCategoria?cmd=nova"><button class="btn" id="btn-nova-categoria">Nova Categoria</button></a>
                  
          <div class="containerTabelaAdmin">       
          <table class="table-custom table-striped" id="tabela-categorias">
            <thead class="thead-color">
              <tr>
                <th scope="col">Cod</th>
                <th scope="col">Nome</th>
                <th scope="col">Foto</th>
                <th scope="col">Brinquedos Cadastrados</th>
            ...
            <tbody>
              <tr>
					<%
					List<Categoria> lista = (List<Categoria>) request.getAttribute("categoriasList");
					if (lista != null) {
					    for (Categoria a : lista) {
					%>
					<tr id="categoriaId<%=a.getId()%>">
					    <td><%=a.getId()%></td>
					    <td><%=a.getNome()%></td>
					    <td id="tdFoto"><img src="<%=a.getUrl_foto()%>" style="width:20px; height:20px;"></td>
					<%
						CategoriaDAO dao;
						dao = new CategoriaDAO();
						int id_categoria = a.getId();
						int totalBrinquedoPorCategoria = dao.TotalBrinquedoPorCategoria(a.getId());
					%>    
					    <td><%=totalBrinquedoPorCategoria%></td>
					    <td><button class="btn" id="btn-editar" onclick="btnEditarCategoria(this)">Editar</button><button class="btn" id="btn-excluir" onclick="btnExcluirCategoria(this)">Excluir</button></td>
					</tr>
					<%
					    }
					} else {
					    // Lide com a lista de categorias nula de acordo com a lógica do seu aplicativo.
